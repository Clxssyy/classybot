...
module.exports = {
  ...
  async execute(client) {
    ...
    const infoChannel = await client.channels.cache.get('1110366740103315516');
    const infoMessages = await infoChannel.messages.fetch();
    const infoBotMessage = infoMessages.find(
      (msg) => msg.author.id === client.user.id && msg.embeds.length === 1
    );

    const featureChannel = await client.channels.cache.get(
      '1110534478188773406'
    );
